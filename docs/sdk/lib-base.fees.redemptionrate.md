...
|  --- | --- | --- |
|  redeemedFractionOfSupply | [Decimalish](./lib-base.decimalish.md) | The amount of aUSD being redeemed divided by the total supply. |
...
Unlike the borrowing rate, the redemption rate depends on the amount being redeemed. To be more precise, it depends on the fraction of the redeemed amount compared to the total aUSD supply, which must be passed as a parameter.

To calculate the redemption fee in aUSD, multiply the redeemed aUSD amount with the redemption rate.

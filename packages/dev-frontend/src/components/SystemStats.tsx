...
const Balances: React.FC = () => {
  ...
  return (
    <Box sx={{ mb: 3 }}>
      ...
      <Statistic name="FTM"> {accountBalance.prettify(4)}</Statistic>
...
export const SystemStats: React.FC<SystemStatsProps> = ({ variant = "info", showBalances }) => {
  ...
  return (
    <Card {...{ variant }}>
      ...
      <Heading>Aquarius statistics</Heading>
      ...
      <Statistic
        name="TVL"
        tooltip="The Total Value Locked (TVL) is the total value of Ether locked as collateral in the system, given in FTM and USD."
      >
        {total.collateral.shorten()} <Text sx={{ fontSize: 1 }}>&nbsp;FTM</Text>
        <Text sx={{ fontSize: 1 }}>
      ...
      <Statistic name="aUSD supply" tooltip="The total aUSD minted by the Aquarius Protocol.">
        {total.debt.shorten()}
      </Statistic>
      {lusdInStabilityPoolPct && (
        <Statistic
          name="aUSD in Stability Pool"
      ...
      <Statistic
        name="Staked AQU"
        tooltip="The total amount of AQU that is staked for earning fee revenue."
      >
        {totalStakedLQTY.shorten()}
      </Statistic>
      <Statistic
        name="Total Collateral Ratio"
        tooltip="The ratio of the Dollar value of the entire system collateral at the current FTM:USD price, to the entire system debt."
      >
      ...
      {kickbackRatePct && (
        <Statistic
          name="Kickback Rate"
          tooltip="A rate between 0 and 100% set by the Frontend Operator that determines the fraction of AQU that will be paid out as a kickback to the Stability Providers using the frontend."
        >
          {kickbackRatePct}%
        </Statistic>
      )}

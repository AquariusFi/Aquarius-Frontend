...
const uniLink = (lusdAddress: string) => `https://spookyswap.finance/add/FTM/${lusdAddress}`;
    ...
    <Card>
      ...
      <Box sx={{ p: [2, 3] }}>
        <InfoMessage title="You aren't farming AQU.">
          <Paragraph>You can farm AQU by staking your Fantom FTM/aUSD LP tokens.</Paragraph>

          <Paragraph sx={{ mt: 2 }}>
            You can obtain LP tokens by adding liquidity to the{" "}
            <Link href={uniLink(addresses["lusdToken"])} target="_blank">
            FTM/AQU pool on SpookySwap. <Icon name="external-link-alt" size="xs" />
            </Link>
          </Paragraph>
        </InfoMessage>

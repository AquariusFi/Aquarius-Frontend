...
export const Yield: React.FC = () => {
  const {
    meContract,
    liquity: {
      connection: { addresses, liquidityMiningLQTY1RewardRate }
    }
  } = useLiquity();

  const { remainingLiquidityMiningLQTY1Reward, totalStakedAusdLpTokens } = useLiquitySelector(selector);
  const [lqtyPrice, setLqtyPrice] = useState<Decimal | undefined>(undefined);
  const [uniLpPrice, setUniLpPrice] = useState<Decimal | undefined>(undefined);
  const hasZeroValue = remainingLiquidityMiningLQTY1Reward.isZero || totalStakedAusdLpTokens.isZero;
  const lqtyTokenAddress = addresses["lqtyToken"];
  const uniTokenAddress = addresses["uniToken"];
  const secondsRemaining = remainingLiquidityMiningLQTY1Reward.div(liquidityMiningLQTY1RewardRate);
  ...
  useEffect(() => {
    // meContract.spookypool.rewardRate().then((res:any) => {
    //   console.log(1222223,Number(res))
    // });
  ...
  const remainingLqtyInUSD = remainingLiquidityMiningLQTY1Reward.mul(lqtyPrice);
  const totalStakedUniLpInUSD = totalStakedAusdLpTokens.mul(uniLpPrice);
      ...
      <InfoIcon
        tooltip={
          <Card variant="tooltip" sx={{ minWidth: ["auto", "352px"] }}>
            <Paragraph>
              An <Text sx={{ fontWeight: "bold" }}>estimate</Text> of the AQU return on staked Spooky
              LP tokens. The farm runs for 6-weeks, and the return is relative to the time remaining.
            </Paragraph>
            <Paragraph sx={{ fontSize: "12px", fontFamily: "monospace", mt: 2 }}>
              ($AQU_REWARDS / $STAKED_SPOOKY_LP) * 100 ={" "}

...
export interface LiquityStoreBaseState {
  ...
  /** User's aUSD token balance. */
  lusdBalance: Decimal;

  /** User's AQU token balance. */
  lqtyBalance: Decimal;
  ...
  /** Total amount of aUSD currently deposited in the Stability Pool. */
  lusdInStabilityPool: Decimal;

  /** Total collateral and debt in the Aquarius system. */
  total: Trove;
  ...
  /** Remaining AQU that will be collectively rewarded to stability depositors. */
  remainingStabilityPoolLQTYReward: Decimal;

  /** Remaining AQU that will be collectively rewarded to stability depositors. */
  // remainingStabilityPoolLQTYOReward: Decimal;
...
export interface LiquityStoreDerivedState {
   ...
   *
   * @remarks
   * Note that the actual rate paid by a redemption transaction will depend on the amount of aUSD
   * being redeemed.
   *
...
/**
 * Abstract base class of Aquarius data store implementations.
 *
 * @remarks
...
export abstract class LiquityStore<T = unknown> {
  ...
  /**
   * Start monitoring the blockchain for Aquarius state changes.
   *
   * @remarks
  ...
  private _reduce(
    ...
    return {
      ...
      lqtyLpTokenBalance: this._updateIfChanged(
        eq,
        "lqtyLpTokenBalance",
        baseState.lqtyLpTokenBalance,
      ...
      remainingLiquidityMiningLQTYOReward: this._silentlyUpdateIfChanged(
        eq,
      ...
      liquidityMiningStakeLqtyLp: this._updateIfChanged(
        eq,
        "liquidityMiningStakeLqtyLp",
        baseState.liquidityMiningStakeLqtyLp,
      ...
      totalStakedLqtyLpTokens: this._updateIfChanged(
        eq,
        "totalStakedLqtyLpTokens",
        baseState.totalStakedLqtyLpTokens,
      ...
      liquidityMiningLQTYLpReward: this._silentlyUpdateIfChanged(
        eq,
      ...
      // remainingStabilityPoolLQTYOReward: this._silentlyUpdateIfChanged(
      //   eq,
      //   baseState.remainingStabilityPoolLQTYOReward,
      //   baseStateUpdate.remainingStabilityPoolLQTYOReward
      // ),

...
export class PopulatableEthersLiquity
  ...
  async setPrice(
    ...
    if (!_priceFeedIsTestnet(priceFeed)) {
      throw new Error("setPrice() unavailable on this deployment of Aquarius");
    }
  ...
  async _mintUniToken(
    ...
    if (!_uniTokenIsMock(uniToken)) {
      throw new Error("_mintUniToken() unavailable on this deployment of Aquarius");
    }
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.approveUniTokens} */
  async approveLqtyLpTokens(
    allowance?: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { lqtyLPToken, lqtyUnipool } = _getContracts(this._readable.connection);
    // const { uniToken, unipool } = _getContracts(this._readable.connection);

    return this._wrapSimpleTransaction(
      await lqtyLPToken.estimateAndPopulate.approve(
        { ...overrides },
        id,
        lqtyUnipool.address,
        Decimal.from(allowance ?? Decimal.INFINITY).hex
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.stakeUniTokens} */
  async stakeLqtyLpTokens(
    amount: Decimalish,
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { lqtyUnipool } = _getContracts(this._readable.connection);
  ...
  async unstakeLqtyLpTokens(
    ...
    return this._wrapSimpleTransaction(
      await lqtyUnipool.estimateAndPopulate.withdraw(
        { ...overrides },
        addGasForUnipoolRewardUpdate,
        Decimal.from(amount).hex
      )
  ...
  /** {@inheritDoc @liquity/lib-base#PopulatableLiquity.withdrawLQTYRewardFromLiquidityMining} */
  async withdrawLQTYORewardFromLiquidityMining(
    overrides?: EthersTransactionOverrides
  ): Promise<PopulatedEthersLiquityTransaction<void>> {
    const { lqtyUnipool } = _getContracts(this._readable.connection);
